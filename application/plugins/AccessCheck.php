<?php

class Application_Plugin_AccessCheck extends Zend_Controller_Plugin_Abstract {

    private $_acl = null;
    private $_auth = null;

    /*
     * Инициализируем данные
     */

    public function __construct(Zend_Acl $acl, Zend_Auth $auth) {
        $this->_acl = $acl;
        $this->_auth = $auth;
    }

    public function preDispatch(Zend_Controller_Request_Abstract $request) {
        // получаем имя текущего ресурса
        $resource = $request->getControllerName();
        
        
        if($resource == 'pre-login') {
            return ;
        }
        
        
        // получаем имя action
        $action = $request->getActionName();
        
        
        $application = new Zend_Application(
                APPLICATION_ENV, APPLICATION_PATH . '/configs/application.ini'
        );

        $identity = $this->_auth->getStorage()->read();
       
        
        if ($application->getOption('writingEvents') == 'on') {
            
            $requesturi = $request->getRequestUri();

            // получаем доступ к хранилищу данных Zend,
            // и достаём роль пользователя

            $data = array(
                'user_id' => $identity->id,
                'action' => "$requesturi",
            );
            $table_events = new Application_Model_Db_Events();
            if (!empty($identity)) {
                $table_events->insert($data);
            }
        }
        // если в хранилище ничего нет, то значит мы имеем дело с гостем
        $role = !empty($identity->role_id) ? $identity->role_id : 'guest';
        // если пользователь не допущен до данного ресурса,
        // то отсылаем его на страницу авторизации 
        if (!$this->_acl->isAllowed($role, $resource, $action)) {
//            $request->setControllerName('pre-login')->setActionName('index');
            
        }
    }

}
